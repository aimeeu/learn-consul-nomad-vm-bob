...
- shared/conf/: Consul, Nomad, Vault, and consul-template agent configuration templates uploaded to VM instances by Ansible.
...
## Standard Workflow
1. Edit variables.hcl from aws/:
   - cp variables.hcl.example variables.hcl
   - Set region and ami (base Ubuntu 22.04 AMI ID for the region; see variables.hcl.example for lookup command)
2. Set CONSUL_TLS_SERVER_NAME before terraform apply:
   - export CONSUL_TLS_SERVER_NAME=consul.dc1.global
3. Provision server infrastructure from aws/ (phase 1 – no consul provider dependencies):
   - terraform init
...
4. Install packages and configure server nodes with Ansible (phase 1):
   - ansible-playbook -i aws/inventory-servers.yml shared/ansible/site.yml
   This installs Docker, Java, and HashiStack packages, uploads configuration
   templates, then starts Consul and Nomad on the server nodes and bootstraps ACLs.
5. Apply remaining Terraform resources from aws/ (phase 2 – consul provider now reachable):
   - terraform apply -var-file=variables.hcl
   ...
   - source ./datacenter.env
6. Install packages and configure client nodes with Ansible (phase 2):
   - ansible-playbook -i aws/inventory.yml shared/ansible/site.yml
7. Deploy Nomad jobs from shared/jobs/ in sequence:
